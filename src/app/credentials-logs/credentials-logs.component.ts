...
@Component({
  selector: 'app-credentials-logs',
  templateUrl: './credentials-logs.component.html',
  styleUrls: ['./credentials-logs.component.css']
})
export class CredentialsLogsComponent {
  logs: any[] = [];
  currentDate: string;
  userArray: any[] = [];
  finalLogsArray: any[] = [];
...
displayedColumns: string[] = ['date', 'userName', 'action'];
  constructor(private store: Store<{ userLogsList: UserLogsState }>, private credentialsService: CredentialsService) {
  ...
  loadAuthenticationLogs(date: string, campus: any | null) {
    this.credentialsService.getCredentialLogs(date, campus).subscribe({
      next: (response) => {
        this.logs = response;
