...
export class ArchiveEffects {
  ...
  loadUserArchiveList$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(this.store.pipe(select(ArchiveSelectors.selectUserArchiveList))),
      mergeMap(([action, userList]) => {
        if (userList.length > 0) {
          // User list is already loaded, return success action with cached data
          return of(ArchiveActions.loadUserArchiveListSuccess({ users: userList }));
        } else {
          return this.archiveService.getUserList().pipe(
            map(users => ArchiveActions.loadUserArchiveListSuccess({ users })),
            catchError(error => of(ArchiveActions.loadUserArchiveListFailure({ error })))
          );
        }
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap(([action, documentList]) => {
        if (documentList.length > 0) {
          ...
          return this.archiveService.getArchives().pipe(
  ...
  loadArchivesWithReceiver$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ArchiveActions.loadUserArchiveListSuccess, ArchiveActions.loadDocumentArchiveListSuccess),
      withLatestFrom(
      ...
      switchMap(([action, users, documents]) => {
        ...
        return of(ArchiveActions.loadArchivesWithReceiverSuccess({ finalArchivesArray }));
      }),
      catchError(error => of(ArchiveActions.loadArchivesWithReceiverFailure({ error })))
    )
  );
}
